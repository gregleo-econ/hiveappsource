...
axios
    .get('http://127.0.0.1:9985/subject/get_experiment_info_for_eligible_sessions?my_email=' + store.state.user.email)
    .then(res => {
...
const start_experiment = function (item) {
    axios
        .post('http://127.0.0.1:9985/subject/start_experiment?my_experiment_id=' + item.id + '&my_email=' + store.state.user.email),
