    ...
    paypal.Buttons({
        ...
        onApprove: function (data, actions) {
            ...
            return actions.order.capture().then(function (details) {
                ...
                axios
                    .post('http://127.0.0.1:9985/researcher/new_payment?researcher_email=' + store.state.user.email + '&amount=' + details.purchase_units[0].amount.value + '&source=paypal')
                    .then(res => {
                        axios
                            .get(
                        ...
                        axios
                            .get(
                                'http://127.0.0.1:9985/users/get_funds_detail?my_email=' +
                                store.state.user.email,
                            )
                            .then((res) => {
